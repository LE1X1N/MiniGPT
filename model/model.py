...
class Attention(nn.Module):
    def __init__(self, args: MiniGPTConfig):
        ...
        self.q_proj = nn.Linear(args.hidden_size, args.num_attention_heads*self.head_dim, bias=False)   # 512 -> 8*64
        self.k_proj = nn.Linear(args.hidden_size, args.num_key_value_heads*self.head_dim, bias=False)   # 512 -> 2*64
        self.v_proj = nn.Linear(args.hidden_size, args.num_key_value_heads*self.head_dim, bias=False)   # 512 -> 2*64
        self.o_proj = nn.Linear(args.num_attention_heads*self.head_dim, args.hidden_size, bias=False)   # 8*64 -> 512
    ...
    def forward(self, x:torch.Tensor, 
        ...
        xq = xq.transpose(1, 2)                          #  (B, L, H_q, d) -> (B, H_q, L, d)
        xk = repeat_kv(xk, self.n_rep).transpose(1, 2)   #  (B, L, H_kv, d) -> (B, H_q, L, d)
        ...
        output = output.transpose(1, 2).reshape(B, L, -1)   # (B, H_q, L, d) -> (B, L, H_q, d) -> (B, L, D)
...
class MiniGPTBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        hidden_states = self.mlp(self.post_attention_layernorm(hidden_states)) + hidden_states  # (B, L, D)
...
class MiniGPTModel(nn.Module):
    ...
    def forward(self, 
        ...
        hidden_states = self.dropout(self.embed_tokens(input_ids))  # (B, L) -> (B, L, D)
        
        position_embeddings = (
            self.freqs_cos[start_pos : start_pos+L],     # (L, d)
