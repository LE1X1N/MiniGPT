    ...
    if model is not None:
        ...
        if os.path.exists(resume_path):
            ...
            if saved_ws != current_ws:
                ...
                Logger(
                    f"Number of GPU changed: ({saved_ws}->{current_ws}), step is converted to {ckp_data['step']}"
                )
            return ckp_data
        return None
...
def init_model(lm_config, from_weight="pretrain", tokenizer_path=None, save_dir="out", device="cuda"):
    ...
    if tokenizer_path is None:
        ...
        project_root = os.path.dirname(current_dir)   # project root path
        tokenizer_path = os.path.join(project_root, "model")
    
    tokenzier = AutoTokenizer.from_pretrained(tokenizer_path)
